...
*Setup instructions for a gitian build of Zapcard using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zapcard
...
- [Building Zapcard](#building-zapcard)
...
Clone the git repositories for zapcard and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zapcard/zapcard
```
...
In [doc/release-process.md](release-process.md) in the zapcard repository under 'Fetch and build inputs'.
...
To build Zapcard (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zapcard repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zapcard/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zapcard/zapcard
...
For example:
```bash
URL=https://github.com/laanwj/zapcard.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit zapcard=${COMMIT} --url zapcard=${URL} ../zapcard/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zapcard=${COMMIT} --url zapcard=${URL} ../zapcard/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/zapcard-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zapcard-build.assert
```
...
[zapcard/gitian.sigs](https://github.com/zapcard/gitian.sigs/) repository, or
